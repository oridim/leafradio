...
export default command({
    name: 'unlink',
    desc: 'Unlinks a repository from a radio.',
    ...
    handler: withEntityManager(
        async (
            ...
            if (!repositories.contains(repository)) {
                console.error(
                    `Invalid value: value for the argument 'radio-identifier' was not linked with argument 'repository-identifier'`,
                );
            ...
            console.log(
                `[LeafRadio] Unlinked radio '${radioID}' ('${name}') with repository '${repositoryID}' ('${directoryPath}').`,
            );
        },
    ),
});
